from transformers import AutoTokenizer,AutoModelForMaskedLM,AlbertTokenizer,RobertaTokenizer, RobertaModel,DataCollatorForLanguageModeling,\
RobertaForMaskedLM,Trainer,TrainingArguments,DataCollatorWithPadding,AlbertForSequenceClassification,RobertaForSequenceClassification,\
...
parser.add_argument('--output_model_dir', type=str, help='模型保存地址', default='./finetune_albert_pretrain_0')
parser.add_argument('--tar_name_prefix', type=str, help='最终模型打包名称', default='roberta_finetune_no_pretrain')
...
def tokenize_function(example):
    return tokenizer(example["question1"],example["question2"],truncation=True,padding="max_length",max_length=args.max_len)
...
# model = RobertaForSequenceClassification.from_pretrained('./pretrain_20')
