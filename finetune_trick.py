...
from transformers import RobertaModel, BertConfig, RobertaTokenizer, DataCollatorWithPadding, AdamW, \
...
parser.add_argument('--pretrained_model_dir', type=str, help='预训练模型地址', default='./roberta-base')
...
parser.add_argument('--loss_type', type=str, help='loss类型', default='ce')
...
parser.add_argument('--output_model_dir', type=str, help='模型保存地址', default='./finetune_trick')
parser.add_argument('--tar_name_prefix', type=str, help='最终模型打包名称', default='finetune_trick')
...
def tokenize_function(example):
    return tokenizer(example["question1"], example["question2"], truncation=True, max_length=args.max_len)
...
for epoch in range(args.num_train_epochs):
    ...
    for step, batch_data in enumerate(train_dataloader):
        ...
        if args.use_fp16:
            ...
            # print(loss_fn)
            print(batch_data)
            print(model(**batch_data,loss_fn=loss_fn))
            loss = model(**batch_data, loss_fn=loss_fn)[1]
